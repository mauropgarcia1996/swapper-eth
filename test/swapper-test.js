...
describe("Swapper", () => {
  describe("Swapper Contract", () => {
    ...
    beforeEach(async () => {
      ...
      const cointTwoToContract = await coinTwo
        .connect(address2)
        .transfer(swapper.address, ethers.BigNumber.from(amount));
    ...
    it("Should provide CoinOne to Swapper", async () => {
      ...
      await provided.wait();
      expect((await coinOne.balanceOf(address1.address)).toString()).to.equal(
        (ethers.BigNumber.from(mintedAmount) - ethers.BigNumber.from(amount)).toString()
      );
    ...
    it("Should return DAI amount from ETH", async () => {
      const daiAmount = await swapper.functions.swapDai(ethers.BigNumber.from(daiAmountString));
